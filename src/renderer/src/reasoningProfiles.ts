...
const EFFORT_INSTRUCTIONS: Record<ReasoningEffort, string> = {
  Instant: 'Do not perform or emit chain-of-thought, hidden analysis, deliberation, or thinking tokens. Answer immediately from the available context. If tools are necessary, select and call them directly without a narrated planning pass. Optimize for the absolute least possible number of tool calls. For observation, inspection, lookup, and research, make at most the single decisive call needed and never call the same or a substantially similar tool again merely to reassure yourself, reconfirm evidence, or explore alternatives. If the answer is already available, call no tool. Return only tool calls or the final answer.',

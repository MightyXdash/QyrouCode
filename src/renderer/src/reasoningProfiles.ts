...
const EFFORT_INSTRUCTIONS: Record<ReasoningEffort, string> = {
  Instant: 'Answer immediately using the obvious fastest reliable path. Optimize for and reward the absolute least possible number of tool calls. For observation, inspection, lookup, and research, make at most the single decisive call needed and never call the same or a substantially similar tool again merely to reassure yourself, reconfirm evidence, or explore alternatives. If the answer is already available, call no tool. Writing and command tools are allowed when necessary, but keep them to the shortest possible sequence with no redundant reads or verification. Organize unavoidable tool work into only 1–4 distinct important substeps and therefore use cur_task_state only 1–4 times. Never use it for routine calls or retries. Be decisive, concise, and avoid tangents.',
  Low: 'Use a short, focused internal reasoning pass and take the fastest sound path. Minimize tool calls aggressively. Do not call the same or a substantially similar observation, inspection, lookup, or research tool merely to reassure yourself or reconfirm an already-supported conclusion. Use one focused verification only when it materially affects correctness. Keep writing and command sequences compact and avoid redundant reads, searches, and post-change checks. Organize tool-based work into about 2–6 distinct important substeps and therefore use cur_task_state about 2–6 times. Never use it for routine calls or retries. Identify the key constraint and avoid unnecessary alternatives.',
  Medium: 'Reason internally through a few plausible approaches and their important tradeoffs. Organize tool-based work into about 3–8 distinct important substeps and therefore use cur_task_state about 3–8 times. Never use it for routine calls or retries. Check the main assumptions, then choose the strongest approach.',
  High: 'Reason deeply and explore multiple viable approaches. Organize tool-based work into about 5–11 distinct important substeps and therefore use cur_task_state about 5–11 times. Never use it for routine calls or retries. Test assumptions, compare tradeoffs, consider meaningful failure modes, then verify the best answer.',
  'Extra high': 'Use the deepest feasible internal analysis. Organize tool-based work into about 8–16 distinct important substeps and therefore use cur_task_state about 8–16 times. Never use it for routine calls or retries. Systematically consider approaches, assumptions, edge cases, failure modes, and success conditions, then verify the answer from multiple angles.'
}

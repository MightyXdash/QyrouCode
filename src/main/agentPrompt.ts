...
cur_task_state is required control-plane progress metadata for agentic work, never assistant prose or a synchronization barrier. Invoke it through the structured function-call channel only. Include exactly one in the first meaningful tool batch in the same response as the action tools it describes, then include a fresh structured call when changing phase, finding something important, beginning a long check, reaching a blocker, or after roughly six actions when work continues. Writing the intended status as ordinary assistant text is invalid and will be discarded. Write one friendly, natural paragraph of 12–63 useful words in the configured native language. Aim for roughly 60 words and normally stay above about 25 words; use 12–24 words only when the action is genuinely simple and small. Refer to the user's specific subject, entity, file, or goal whenever natural, without merely repeating their entire prompt. Explain what you are about to do and why it helps instead of exposing a terse tool label or generic phrase such as "your question." Do not reveal private reasoning, repeat information already obvious from tool labels, call cur_task_state alone, or add one to a final-answer turn.
